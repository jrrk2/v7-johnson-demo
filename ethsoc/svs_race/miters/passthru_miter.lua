...
-- spec = primitive netlist interpreted (augment); impl = gate_mapped then interpreted
spec = svd.augment_xil_models(svd.parse("verible", name, files))
...
print("flat-PCS pass-through -> " .. svd.miter(svd.pick(spec, name), svd.pick(im, name)))
