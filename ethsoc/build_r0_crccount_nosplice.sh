#!/bin/bash
# R0 open flow, SPLICE-FREE GT variant.  Identical to build_r0_crccount.sh up to
...
$YOSYS $ETH/r0_crccount_nosplice.ys > /tmp/r0_crccount_yosys.log 2>&1 \
  || { echo "YOSYS FAILED"; tail -20 /tmp/r0_crccount_yosys.log; exit 1; }
echo "yosys OK"
python3 $ETH/stamp_placement.py /tmp/r0_crccount.json $ETH/placement_crccount.txt /tmp/r0_crccount_stamped.json
...
flock /tmp/nextpnr.lock env NEXTPNR_ARC_MAX_VISIT=2000000 ${NEXTPNR_EXTRA_ENV:+$NEXTPNR_EXTRA_ENV} NEXTPNR_ALLOW_CO_5FF_CONTENTION=1 \
  ...
  $NEXTPNR --chipdb $CHIPDB --xdc ${ETHLOOP_XDC:-$ETH/r0_pins.xdc} --json /tmp/r0_crccount_stamped.json \
    --fasm /tmp/r0_crccount.fasm --write /tmp/r0_crccount_routed.json --freq 50 --router router2 --placer sa \
    > /tmp/r0_crccount_npnr.log 2>&1 || { echo "NEXTPNR FAILED"; tail -25 /tmp/r0_crccount_npnr.log; exit 1; }
grep -iE "Max frequency|Routing complete|failed|unrouted" /tmp/r0_crccount_npnr.log | tail -5
echo "nextpnr OK"
...
GTCOMMON=$(grep -oE "GTX_COMMON_X[0-9]+Y[0-9]+" /tmp/r0_crccount.fasm | head -1)
cp /tmp/r0_crccount.fasm /tmp/r0_crccount_nosplice.fasm
if [ -n "$GTCOMMON" ]; then
  printf '%s.IBUFDS_GTE2_Y0.CLKCM_CFG\n%s.IBUFDS_GTE2_Y0.CLKRCV_TRST\n' "$GTCOMMON" "$GTCOMMON" \
    >> /tmp/r0_crccount_nosplice.fasm
  echo "injected IBUFDS_GTE2 defaults for $GTCOMMON"
fi

XRAY_ALLOW_MISSING_FEATURES=1 $PXPY $HOME/prjxray/utils/fasm2frames.py \
  --db-root $PXDB --part $PART /tmp/r0_crccount_nosplice.fasm /tmp/r0_crccount.frames > /tmp/r0_crccount_f2f.log 2>&1 \
  && echo "frames OK (GT from DB, no splice)" || { echo "F2F FAILED"; tail /tmp/r0_crccount_f2f.log; exit 1; }

$HOME/prjxray/build/tools/xc7frames2bit --part_file $PXDB/$PART/part.yaml \
  --part_name $PART --frm_file /tmp/r0_crccount.frames --output_file /tmp/r0_crccount_nosplice.bit \
  > /tmp/r0_crccount_f2b.log 2>&1 && echo "bit OK" || { echo "F2B FAILED"; tail /tmp/r0_crccount_f2b.log; exit 1; }
ls -l /tmp/r0_crccount_nosplice.bit | awk '{print "NOSPLICE BIT:",$5,"bytes"}'
