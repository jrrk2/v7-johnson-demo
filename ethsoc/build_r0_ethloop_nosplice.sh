#!/bin/bash
# R0 open flow, SPLICE-FREE GT variant.  Identical to build_r0_ethloop.sh up to
...
$YOSYS $ETH/r0_ethloop_nosplice.ys > /tmp/r0_ethloop_yosys.log 2>&1 \
...
GTCOMMON=$(grep -oE "GTX_COMMON_X[0-9]+Y[0-9]+" /tmp/r0_ethloop.fasm | head -1)
cp /tmp/r0_ethloop.fasm /tmp/r0_ethloop_nosplice.fasm
if [ -n "$GTCOMMON" ]; then
  printf '%s.IBUFDS_GTE2_Y0.CLKCM_CFG\n%s.IBUFDS_GTE2_Y0.CLKRCV_TRST\n' "$GTCOMMON" "$GTCOMMON" \
    >> /tmp/r0_ethloop_nosplice.fasm
  echo "injected IBUFDS_GTE2 defaults for $GTCOMMON"
fi

XRAY_ALLOW_MISSING_FEATURES=1 $PXPY $HOME/prjxray/utils/fasm2frames.py \
  --db-root $PXDB --part $PART /tmp/r0_ethloop_nosplice.fasm /tmp/r0_ethloop.frames > /tmp/r0_ethloop_f2f.log 2>&1 \
  && echo "frames OK (GT from DB, no splice)" || { echo "F2F FAILED"; tail /tmp/r0_ethloop_f2f.log; exit 1; }

$HOME/prjxray/build/tools/xc7frames2bit --part_file $PXDB/$PART/part.yaml \
  --part_name $PART --frm_file /tmp/r0_ethloop.frames --output_file /tmp/r0_ethloop_nosplice.bit \
  > /tmp/r0_ethloop_f2b.log 2>&1 && echo "bit OK" || { echo "F2B FAILED"; tail /tmp/r0_ethloop_f2b.log; exit 1; }
ls -l /tmp/r0_ethloop_nosplice.bit | awk '{print "NOSPLICE BIT:",$5,"bytes"}'
