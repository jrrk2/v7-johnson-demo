...
`endif
    ...
    wire rst_all = rst_buf | (por_cnt != 5'd0);
...
`endif
    ...
    wire [7:0] rxf_dout, core_tx, txf_dout, led_int;
    wire core_rx_rd, rxf_empty, rxf_full, core_tx_stb, txf_empty, txf_full;
    byte_fifo rxfifo (.clk(clk),.rst(rst_all),.wr(rxdone),.din(rxbyte),.full(rxf_full),
        .rd(core_rx_rd),.dout(rxf_dout),.empty(rxf_empty));
    ...
    always @(posedge clk or posedge rst_all)
        if (rst_all) begin tst<=T_IDLE; tsr<=0; txstart<=0; txf_rd<=0; end
        else begin
            txstart<=1'b0; txf_rd<=1'b0;
            case (tst)
                T_IDLE:  if (!txf_empty) begin tsr<=txf_dout; txf_rd<=1'b1; txstart<=1'b1; tst<=T_START; end
    ...
    uart_transmitter txu (.CLK(clk),.RST(rst_all),.TXCLK(baudtick2x),
        ...
        .EPS(1'b0),.SP(1'b0),.BC(1'b0),.DIN(tsr),.TXFINISHED(txfinished),.SOUT(sout));
    assign tx_int = sout;
